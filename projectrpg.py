...
def open_custom_skill_window():
    """
    Window for creating custom spells by combining learned magic with custom mana costs.
    """
    custom_window = tk.Toplevel(window)
    custom_window.title("Création de compétences personnalisées")
    custom_window.geometry("850x750")
    custom_window.transient(window)
    custom_window.grab_set()
    custom_window.focus_set()
    custom_window.configure(bg=COLOR_LIGHT_BG_BLUE)

    # Title
    tk.Label(custom_window, text="🔮 Création de compétences personnalisées", 
             font=FONT_WINDOW_TITLE, bg=COLOR_LIGHT_BG_BLUE).pack(pady=15)

    # Main container
    main_container = tk.Frame(custom_window, bg=COLOR_LIGHT_BG_BLUE)
    main_container.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

    # Left side: Available spells list
    left_frame = tk.Frame(main_container, bg=COLOR_LIGHT_BG_BLUE)
    ...
    for fusion in fusion_results:
        for key, val in fusion_recipes.items():
            if isinstance(val, dict) and val.get("result") == fusion:
                # Only include skills (not consumables or enchanted items)
                if not val.get("consumable", False) and not val.get("enchanted", False):
    ...
    spell_canvas.configure(yscrollcommand=spell_scrollbar.set)
    spell_canvas.create_window((0, 0), window=spell_frame, anchor="nw")
    spell_frame.bind("<Configure>", lambda e: spell_canvas.configure(scrollregion=spell_canvas.bbox("all")))
    ...
    mana_slider = tk.Scale(mana_slider_frame, from_=0, to=50, orient=tk.HORIZONTAL, 
                           bg=COLOR_LIGHT_BG_BLUE, length=200)
    mana_slider.set(25)
    mana_slider.pack(side=tk.LEFT, fill=tk.X, expand=True)

    mana_value_label = tk.Label(mana_slider_frame, text="25", font=FONT_WINDOW_SMALL, 
                               bg=COLOR_LIGHT_BG_BLUE, width=3)
    mana_value_label.pack(side=tk.LEFT, padx=10)

    def update_mana_label(val):
        mana_value_label.config(text=str(int(float(val))))
    ...
    def create_custom_spell():
        ...
        if not custom_name:
            messagebox.showwarning("Erreur", "Veuillez entrer un nom pour le sortilège!")
            return
        ...
        if custom_spell_name not in fusion_results:
            ...
            messagebox.showwarning("Sortilège existant", "Ce sortilège existe déjà!")

    create_button_widget = tk.Button(
        right_frame,
        text="✨ Créer le sortilège",
        font=FONT_BUTTON_LARGE,
        bg="#ffaa00",
        fg="white",
        command=create_custom_spell
    )
    create_button_widget.pack(pady=20, padx=10, fill=tk.X)
...
create_button(player_frame, "🎓  Création de compétences", open_custom_skill_window, "magic")
